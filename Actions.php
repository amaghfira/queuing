...
session_set_cookie_params(0); // Cookie expires when the browser is closed
...
Class Actions extends DBConnection{
    ...
    function save_queue(){
        ...
        $_POST['queue'] = $code;
        extract($_POST);
        $sql = "INSERT INTO `queue_list` (`queue`,`customer_name`,`jenis_layanan`) VALUES('{$queue}','{$customer_name}','{$jenis_layanan}')";
        $save = $this->query($sql);
        if($save){
            $resp['status'] = 'success';
